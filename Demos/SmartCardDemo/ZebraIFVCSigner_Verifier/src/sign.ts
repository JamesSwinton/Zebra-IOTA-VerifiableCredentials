...
const CREDENTIALFILE = `${WORKDIR}/credential_${CurrentTime}.json`;
const QRCREDENTIALFILE = `${WORKDIR}/qr_credential_${CurrentTime}.svg`;
...
getIdentity().then((identity) => {
    ...
    createCredential(identity, schemaName as SchemaNames, JSON.parse(fs.readFileSync(dataPath, 'utf8'))).then((credential) => {
        ...
        qrcode.save(QRCREDENTIALFILE, (error: Error) => {
            ...
            console.info('Qr stored!');
        });
        if (!fs.existsSync(WORKDIR)) {
            fs.mkdirSync(WORKDIR);
        }
        fs.writeFile(CREDENTIALFILE, strigifiedCredential, (err: Error) => {
            if (err) return console.error(err);
